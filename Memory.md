...
| 2026-07-18 | **Tab-Bar-Nachbesserung (Build 2).** JB meldet: Logo top, aber Bar sass auf dem iPhone unverändert hoch (der safe-area-Fix von 17.07. griff auf dem Gerät nicht; Ursache dort unklar, evtl. auch alte index.html aus dem Cache). Neuer Ansatz ohne Formel-Raten: (1) `#tabbar` bottom fest `16px` (kein env()/clamp mehr), `#main` padding-bottom fest 78px; Inhalt/Karten dehnen sich automatisch bis zur tiefer sitzenden Bar. (2) JB-Wunsch Indicator: Der schiebbare hellgraue Aktiv-Bereich in der Pille hat neu **2.5px statt 5px Abstand** zum Bar-Aussenrand (CSS top/bottom 2.5px; JS expandiert links/rechts um 2.5px über den Tab-Button hinaus, in `repositionTabIndicator` und `followFinger`). Bar selbst unverändert (Höhe 62, Breite, Stil). (3) **Versionszeile in den Einstellungen** («Forma · Stand 17.07.2026 · Build 2», Konstante `APP_BUILD`) als Cache-Diagnose: Steht dort keine oder eine ältere Version, lädt das iPhone noch die alte Datei. Headless verifiziert: bottom 16px unabhängig vom safe-area-Wert (34 und 96 getestet), Indicator rundum 2.5px, Tab-Wechsel und Drag ok. **Upload: nur `index.html`** neu auf GitHub (Icons/README von 17.07. bleiben gültig). |
